...
import
  ...
  ../beacon_chain/[conf, filepath, interop],
...
type
  ...
  CliConfig* = object
    ...
    of StartUpCommand.createTestnet:
      ...
      eip4844ForkEpoch* {.
        defaultValue: FAR_FUTURE_EPOCH
        desc: "The epoch of the EIP-4844 hard-fork"
        name: "eip-4844-fork-epoch" .}: Epoch
...
func `as`(blk: BlockObject, T: type eip4844.ExecutionPayloadHeader): T =
  T(parent_hash: blk.parentHash as Eth2Digest,
    fee_recipient: blk.miner as ExecutionAddress,
    state_root: blk.stateRoot as Eth2Digest,
    receipts_root: blk.receiptsRoot as Eth2Digest,
...
proc doCreateTestnet*(config: CliConfig,
  ...
  let genesisValidatorsRoot =
    if config.eip4844ForkEpoch == 0:
      createAndSaveState(genesisBlock as eip4844.ExecutionPayloadHeader)
