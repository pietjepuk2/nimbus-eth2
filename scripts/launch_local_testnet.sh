...
if [[ "${RUN_GETH}" == "1" ]]; then
  ...
  if [[ $EIP4844_FORK_EPOCH -lt $STOP_AT_EPOCH ]]; then
...
SHARDING_FORK_TIME=$((GENESIS_TIME + SECONDS_PER_SLOT * SLOTS_PER_EPOCH * EIP4844_FORK_EPOCH))
...
./build/local_testnet_simulation createTestnet \
  ...
  --eip-4844-fork-epoch=$EIP4844_FORK_EPOCH \
